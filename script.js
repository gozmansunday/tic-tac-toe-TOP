...
const Player = (() => {
  ...
  return {playerXDisp, playerODisp, playerX, playerO};
})();

// Module to control flow of game
const GameFlow = (() => {
  const gameBoard = GameBoard.gameBoard;
  const gameInfo = GameBoard.gameInfo;
  const playAgainBtn = GameBoard.playAgainBtn;
  ...
  const showMark = () => {
    gameBoard.addEventListener('click', (e) => {
      const playerXDisp = Player.playerXDisp;
      const playerODisp = Player.playerODisp;
      const playerXName = playerXDisp.textContent;
      const playerOName = playerODisp.textContent;
      let box = e.target;
